...
def _render(
    ...
    out_path = out_dir / f"glyphforge_{theme_name}_{ratio_label.replace(':', 'x')}_{ts}.png"
    save_png(result.image, out_path, dpi=300)

    metrics_text = json.dumps(result.metrics, indent=2)
    return result.image, str(out_path), metrics_text
...
"""
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                ratio_input = gr.Dropdown(choices=RATIO_CHOICES, value="4:5", label="Aspect Ratio")
                density_input = gr.Slider(0.2, 0.95, value=0.65, step=0.01, label="Density")
                seed_input = gr.Number(value=42, precision=0, label="Seed")
                long_edge_input = gr.Slider(960, 3200, value=1600, step=32, label="Long Edge (px)")
                min_font_input = gr.Slider(8, 48, value=12, step=1, label="Min Font Size")
                max_font_input = gr.Slider(18, 110, value=64, step=1, label="Max Font Size")
                attempts_input = gr.Slider(30, 500, value=180, step=5, label="Attempts/Word")
        ...
        render_btn.click(
            ...
            outputs=[output_image, output_file, output_metrics],
        )
