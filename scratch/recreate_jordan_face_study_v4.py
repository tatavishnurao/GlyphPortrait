...
def render_anchor_layer(layer: Image.Image, spec: dict, crop_box: tuple[int, int, int, int], face_mask: np.ndarray, protected: np.ndarray, stats: RenderStats) -> Image.Image:
    ...
    for anchor in spec["anchor_placements"]:
        ...
        alpha = int(np.clip(int(anchor.get("alpha", 210)) + 18, 190, 240))
...
def side_by_side(ref: Image.Image, rec: Image.Image) -> Image.Image:
    ...
    d.text((ref.width + 12, 12), "face study v4", font=font(18, True), fill=(245, 245, 245))
    return out
...
def render() -> None:
    ...
    final.save(OUT / "face_study_v4.png")
    side_by_side(ref_img, final).save(OUT / "face_study_v4_side_by_side.png")
    contours_vis.save(OUT / "face_study_v4_contours.png")
    lane_overlay.save(OUT / "face_study_v4_lane_overlay.png")
    anchor_overlay.save(OUT / "face_study_v4_anchor_overlay.png")
    ...
    (OUT / "face_study_v4_metrics.json").write_text(json.dumps(metrics, indent=2) + "\n")
