...
ANCHORS = [
    ("MICHAEL JORDAN", 1350, 168, 34, (210, 210, 216), 138, -4.0, "head"),
    ("MVP", 1410, 252, 52, (224, 224, 228), 152, -2.0, "head"),
    ("CHAMPION", 1350, 418, 34, (202, 202, 210), 132, -5.0, "head"),
    ("BULLS", 1306, 692, 144, (228, 18, 28), 222, -10.0, "jersey"),
    ("23", 1495, 810, 184, (242, 234, 214), 226, -7.0, "jersey"),
]
...
def _build_region_masks(shape: np.ndarray, jersey: np.ndarray) -> dict[str, np.ndarray]:
    x0, y0, x1, y1 = _bbox(shape)
    h = y1 - y0
    yy = np.broadcast_to(np.arange(OUT_H)[:, None], shape.shape)
    xx = np.broadcast_to(np.arange(OUT_W), shape.shape)
    upper = yy < y0 + int(h * 0.58)
    neck_band = (yy < y0 + int(h * 0.70)) & (xx > x0 + int((x1 - x0) * 0.20)) & (xx < x0 + int((x1 - x0) * 0.66))
    head_neck = (shape > 0) & (jersey == 0) & (upper | neck_band)
    head_neck = ndi.binary_closing(head_neck, structure=np.ones((9, 9), dtype=bool), iterations=1)
    shoulder_body = (shape > 0) & (jersey == 0) & ~head_neck
    return {
        "head_neck": head_neck.astype(np.uint8) * 255,
        "jersey": jersey,
        "shoulder_body": shoulder_body.astype(np.uint8) * 255,
    }


def _contrast_stretch(values: np.ndarray, mask: np.ndarray) -> np.ndarray:
    inside = values[mask > 0]
    if inside.size == 0:
        return values.astype(np.float32)
    lo, hi = np.percentile(inside, [2.5, 98.5])
    stretched = (values - lo) / max(1e-6, hi - lo)
    return np.clip(stretched, 0.0, 1.0).astype(np.float32)


def _hysteresis_edges(strength: np.ndarray, mask: np.ndarray) -> np.ndarray:
    inside = strength[mask > 0]
    high = np.percentile(inside, 87.0)
    low = np.percentile(inside, 70.0)
    strong = (strength >= high) & (mask > 0)
    weak = (strength >= low) & (mask > 0)
    grown = strong.copy()
    for _ in range(3):
        grown = weak & ndi.binary_dilation(grown, structure=np.ones((3, 3), dtype=bool))
    return grown
...
def _build_maps(rgb: np.ndarray, shape: np.ndarray) -> dict[str, np.ndarray]:
    gray = _luma(rgb)
    tone = gray / 255.0
    sculpt_luma = _contrast_stretch(tone, shape)
    ...
    dog = np.abs(ndi.gaussian_filter(gray, sigma=0.7) - ndi.gaussian_filter(gray, sigma=2.0))
    dog = dog / max(1e-6, float(dog.max()))
    edge_strength = np.maximum(mag, dog * 0.75) * (shape > 0)
    canny_like = _hysteresis_edges(edge_strength, shape)
    local_max = edge_strength >= ndi.maximum_filter(edge_strength, size=5)
    contour = (canny_like & local_max) | ((edge_strength > np.percentile(edge_strength[shape > 0], 91)) & (shape > 0))
    contour = ndi.binary_dilation(contour, structure=np.ones((2, 2), dtype=bool), iterations=1)
    feature = contour.astype(np.float32) * edge_strength
    feature = np.clip(feature / max(1e-6, np.percentile(feature[feature > 0], 97) if np.any(feature > 0) else 1.0), 0.0, 1.0)
    highlight = np.clip((tone - 0.34) / 0.48, 0.0, 1.0) * (shape > 0)
    shadow = np.clip((0.25 - tone) / 0.25, 0.0, 1.0) * (shape > 0)
    return {
        "gray": gray.astype(np.float32),
        "tone": tone.astype(np.float32),
        "sculpt_luma": sculpt_luma,
...
def _render_head_detail(target: np.ndarray, head_neck: np.ndarray, maps: dict[str, np.ndarray], rng: random.Random) -> tuple[Image.Image, int, int]:
    layer = Image.new("RGBA", (OUT_W, OUT_H), (0, 0, 0, 0))
    detail = ((maps["feature"] > 0.08) | ((maps["tone"] < 0.22) & (maps["tone"] > 0.055))) & (head_neck > 0)
    ys, xs = np.where(detail)
    if xs.size == 0:
        return layer, 0, 0
    ...
    count = 0
    alpha_sum = 0
    for x, y in coords[:3000]:
        if rng.random() > min(0.72, 0.22 + float(maps["feature"][y, x]) * 0.7 + max(0.0, 0.24 - float(maps["tone"][y, x]))):
            continue
        x = int(x + rng.randint(-8, 8))
        y = int(y + rng.randint(-6, 6))
        if not _valid_point(head_neck, x, y):
            continue
        tone = float(maps["tone"][y, x])
        feature = float(maps["feature"][y, x])
        v = int(np.clip(54 + tone * 120 + feature * 62 + rng.randint(-8, 12), 48, 190))
        alpha = int(np.clip(84 + feature * 88 + max(0.0, 0.24 - tone) * 130 + rng.randint(-14, 16), 74, 180))
        size = rng.randint(9, 17)
        angle = _tangent_angle(maps, x, y) + rng.uniform(-4, 4) if feature > 0.1 else rng.uniform(-5, 5)
        _draw_text(layer, x, y, rng.choice(FACE_WORDS), size, (v, v, min(255, v + 5)), alpha, angle, bold=feature > 0.18)
        count += 1
        alpha_sum += alpha
    return _clip_layer_to_mask(layer, head_neck), count, alpha_sum
...
def _render_jersey_anchor_grain(
    ...
    layer = Image.new("RGBA", (OUT_W, OUT_H), (0, 0, 0, 0))
    mask = ((anchor_mask > 0) & (jersey > 0)).astype(np.uint8) * 255
    ys, xs = np.where(mask > 0)
    if xs.size == 0:
        return layer, 0, 0
    count = 0
    alpha_sum = 0
    for y in range(max(0, int(ys.min())) + rng.randrange(0, 8), min(OUT_H, int(ys.max()) + 1), 8):
        for x in range(max(0, int(xs.min())) + rng.randrange(0, 12), min(OUT_W, int(xs.max()) + 1), rng.choice((10, 12, 14))):
            jx = x + rng.randint(-7, 7)
            jy = y + rng.randint(-6, 6)
            if not _valid_point(mask, jx, jy) or rng.random() > 0.58:
                continue
            tone = float(maps["tone"][jy, jx])
            red = float(red_strength[jy, jx])
            color = _jersey_color(target, red, tone, jx, jy, rng)
            if rng.random() < 0.2:
                v = int(np.clip(120 + tone * 110, 120, 230))
                color = (v, v, v)
            alpha = int(np.clip(60 + red * 54 + tone * 36 + rng.randint(-8, 13), 56, 136))
...
def _final_blend(
    ...
    target: np.ndarray,
    shape: np.ndarray,
    head_neck: np.ndarray,
    anchor_protect: np.ndarray,
    maps: dict[str, np.ndarray],
) -> Image.Image:
    arr = np.array(rendered.convert("RGB"), dtype=np.float32)
    tone = maps["sculpt_luma"][..., None]
    raw_tone = maps["tone"][..., None]
    mask = (shape > 0)[..., None]
    head = (head_neck > 0)[..., None]
    protected = (anchor_protect > 0)[..., None]
    boost = 0.44 + tone * 1.38 + maps["feature"][..., None] * 0.18
    head_boost = 0.58 + tone * 1.10 + maps["feature"][..., None] * 0.20
    boost = np.where(head, head_boost, boost)
    boost = np.where(protected, 0.76 + tone * 0.60, boost)
    boost = np.clip(boost, 0.30, 1.58)
    arr = np.where(mask, arr * boost, arr)
    target_f = target.astype(np.float32)
    highlight = ((raw_tone > 0.58) & mask & ~protected).astype(np.float32)
    arr = arr * (1.0 - highlight * 0.12) + target_f * (highlight * 0.12)
    deep = ((raw_tone < 0.09) & mask & ~head & ~protected).astype(np.float32)
    head_deep = ((raw_tone < 0.11) & head & ~protected).astype(np.float32)
    arr *= 1.0 - deep * 0.34
    arr *= 1.0 - head_deep * 0.16
...
def main() -> None:
    ...
    regions = _build_region_masks(shape, jersey)
    ...
    pass_head, head_words, head_alpha = _render_head_detail(target, regions["head_neck"], maps, rng)
    ...
    pass_a = _reduce_under_anchor(pass_a, head_reserve)
    pass_b = _reduce_under_anchor(pass_b, head_reserve)
    pass_c = _reduce_under_anchor(pass_c, head_reserve)
    anchor_protect = np.maximum(head_reserve, jersey_reserve)

    slogan_layer = Image.new("RGBA", (OUT_W, OUT_H), (0, 0, 0, 0))
    _draw_text(slogan_layer, 795, 514, "change the game.", 28, (188, 188, 202), 225)

    pre_final = _composite_on_black([pass_a, pass_b, pass_head, pass_c, pass_d, pass_e, pass_anchor_grain, slogan_layer])
    final = _final_blend(pre_final, target, shape, regions["head_neck"], anchor_protect, maps)
    ...
    _save_map(out_dir / "one_shot_v3_shape_mask.png", shape)
    _save_map(out_dir / "one_shot_v3_head_neck_mask.png", regions["head_neck"])
    _save_map(out_dir / "one_shot_v3_jersey_mask.png", regions["jersey"])
    _save_map(out_dir / "one_shot_v3_shoulder_body_mask.png", regions["shoulder_body"])
    _save_map(out_dir / "one_shot_v3_feature_contours.png", maps["feature"] * 255.0)
    _save_map(out_dir / "one_shot_v3_luminance_map.png", maps["sculpt_luma"] * 255.0)
    final.save(out_dir / "one_shot_v3_hybrid_final.png")
    side.save(out_dir / "one_shot_v3_hybrid_side_by_side.png")

    jersey_words += anchor_grain_words
    alpha_sum = shadow_alpha + tone_alpha + head_alpha + contour_alpha + jersey_alpha + anchor_alpha + anchor_grain_alpha
    total_words = shadow_words + tone_words + head_words + contour_words + jersey_words + anchor_words
    metrics = {
        "target_path": str(_target_path()),
        "total_words_drawn": total_words,
        "shadow_words": shadow_words,
        "tone_words": tone_words,
        "head_detail_words": head_words,
        ...
        "head_neck_mask_coverage": round(float(np.mean(regions["head_neck"] > 0)), 5),
        "shoulder_body_mask_coverage": round(float(np.mean(regions["shoulder_body"] > 0)), 5),
        "contour_feature_coverage": round(float(np.mean(maps["feature"] > 0)), 5),
    ...
    (out_dir / "one_shot_v3_metrics.json").write_text(json.dumps(metrics, indent=2), encoding="utf-8")
